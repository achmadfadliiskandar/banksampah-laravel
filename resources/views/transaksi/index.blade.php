...
@section('content')
                ...
                <div class="card border-0 shadow-sm mb-4 border-start border-primary border-4">
                    ...
                    <div class="card-body">
                        <form action="{{ route('transaksi.scan-ai') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <small class="text-muted"><i class="fas fa-info-circle"></i> Upload foto sampah, AI akan
                                otomatis memilihkan kategorinya.</small>
                        </form>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <form action="{{ route('transaksi.store') }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" id="nasabahInput" class="form-control"
                                    placeholder="Ketik nama nasabah..." list="dataNasabah" required autocomplete="off">
                                    ...
                                    <div class="col-md-6 pe-1">
                                        <select name="kategori_id[]" class="form-control" required>
                                            ...
                                            @foreach ($kategori as $k)
                                                <option value="{{ $k->id }}"
                                                    {{ session('auto_select_id') == $k->id ? 'selected' : '' }}>
                                                    {{ $k->nama_jenis }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                            ...
                            <button type="button" id="add-item" class="btn btn-outline-secondary btn-sm mb-4 w-100"><i
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover align-middle mb-0" id="tabel">
                                ...
                                <tbody>
                                    @forelse($setoran as $s)
                                        <tr>
                                            <td class="ps-3">
                                                ...
                                                <small
                                                    class="text-muted">{{ $s->created_at->format('d M Y, H:i') }}</small>
                                                <br>
                                                <span
                                                    class="badge {{ $s->status == 'success' ? 'bg-success' : 'bg-warning text-dark' }} mt-1">Status:
                                            ...
                                            <td>
                                                @foreach ($s->details as $detail)
                                                    <span class="badge bg-light text-dark border mb-1">
                                                        {{ number_format($detail->kategori->harga_per_kg * $detail->berat, 0, ',', '.') }}
                                                    </span><br>
                                                @endforeach
                                            </td>
                                            <td>{{ $s->total_berat }} Kg</td>
                                            <td class="fw-bold text-success">Rp
                                                {{ number_format($s->total_harga, 0, ',', '.') }}</td>
                                            <td class="text-center">
                                                @php
                                                    $selisihHari = \Carbon\Carbon::parse($s->created_at)->diffInDays(
                                                        \Carbon\Carbon::now(),
                                                    );
                                                ...
                                                <div
                                                    class="p-1 rounded-3 {{ $wajibDihapus ? 'border border-danger border-2 bg-danger-subtle' : '' }}">
                                                    @if ($wajibDihapus)
                                                        <form action="{{ route('transaksi.destroy', $s->id) }}"
                                                            method="POST">
                                                            @csrf @method('DELETE')
                                                            <button type="submit"
                                                                class="btn btn-sm btn-danger w-100 fw-bold shadow-sm"
                                                                onclick="return confirm('Hapus data fiktif?')">
                                                        ...
                                                        <div class="d-flex gap-1 justify-content-center">
                                                            ...
                                                            @if ($s->status == 'pending')
                                                                <button type="button"
                                                                    ...
                                                                    data-bs-target="#modalSetujuTrx-{{ $s->id }}">
                                                            ...
                                                            <div class="dropdown">
                                                                <button
                                                                    class="btn btn-sm {{ $s->status == 'success' ? 'btn-success text-white' : 'btn-secondary' }} dropdown-toggle fw-bold"
                                                                ...
                                                                <ul
                                                                    ...
                                                                    <li>
                                                                        <button class="dropdown-item text-primary"
                                                                            ...
                                                                            data-bs-target="#modalDetailTrx-{{ $s->id }}">
                                                                            <i class="fas fa-edit fa-fw me-2"></i> Koreksi
                                                                            & Detail Sampah
                                                                        </button>
                                                                    </li>
                                                                    ...
                                                                    @if ($s->status == 'pending')
                                                                        ...
                                                                        <li>
                                                                            <form
                                                                                action="{{ route('transaksi.destroy', $s->id) }}"
                                                                                method="POST"
                                                                                id="form-tolak-{{ $s->id }}">
                                                                                @csrf @method('DELETE')
                                                                                <button type="button"
                                                                                    class="dropdown-item text-danger"
                                                                                    onclick="if(confirm('Tolak & hapus transaksi ini?')) document.getElementById('form-tolak-{{ $s->id }}').submit();">
                                                                    ...
                                                                    @elseif ($s->status == 'success' && $selisihHari < 2)
                                                                        ...
                                                                        <li>
                                                                            <form
                                                                                ...
                                                                                <button type="button"
                                                                                    class="dropdown-item text-danger"
                                                                                    onclick="if(confirm('Batalkan transaksi ini? Saldo nasabah akan dipotong kembali.')) document.getElementById('form-batal-{{ $s->id }}').submit();">
                                                                                    <i class="fas fa-undo fa-fw me-2"></i>
                                                                                    Batalkan & Potong Saldo
                                                                                </button>
                                                                            </form>
                                                                        </li>
                                                                    @endif
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="6" class="text-center py-5 text-muted">Belum ada transaksi
                                                setoran.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
    ...
    @foreach ($setoran as $s)
        ...
        <div class="modal fade" id="modalDetailTrx-{{ $s->id }}" tabindex="-1"
            aria-labelledby="modalDetailLabel-{{ $s->id }}" aria-hidden="true" data-bs-backdrop="static">
                    ...
                    <form action="{{ route('transaksi.update', $s->id) }}" method="POST">
                        ...
                        <div class="modal-body p-3 text-dark">
                            <div class="card card-body bg-light border-0 shadow-sm mb-3 py-2">
                                <div class="row small">
                                    <div class="col-md-6">
                                        <strong>Nama Nasabah:</strong> <span
                                            class="text-secondary">{{ $s->user->name }}</span>
                                    </div>
                            ...
                            <h6 class="fw-bold text-primary mb-3"><i class="fas fa-table me-1"></i> Tabel Timbangan Fisik
                            </h6>
                                                ...
                                                <tr>
                                                    <td>
                                                        ...
                                                        @else
                                                            <div class="text-muted fst-italic"
                                                                ...
                                                                <i class="fas fa-image-slash d-block mb-1 fa-lg"></i> No
                                                                Image
                                                            </div>
                                                        @endif
                                                    </td>
                                                    <td class="text-start">
                                                        ...
                                                        <select name="kategori_id[]"
                                                            class="form-select form-select-sm fw-semibold text-secondary"
                                                            required>
                                                            @foreach ($kategori as $k)
                                                                <option value="{{ $k->id }}"
                                                                    {{ $detail->kategori_id == $k->id ? 'selected' : '' }}>
                                                                    {{ $k->nama_jenis }} (Rp
                                                                    {{ number_format($k->harga_per_kg, 0, ',', '.') }}/Kg)
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    </td>
                                                    <td>
                                                        <div class="input-group input-group-sm">
                                                            <input type="number" step="0.01" name="berat[]"
                                                                ...
                                                                value="{{ number_format($detail->berat, 2, '.', '') }}"
                                                                min="0.01" required>
                                                            <span
                                                                class="input-group-text bg-secondary text-white">Kg</span>
                                                        </div>
                                                    </td>
                                                </tr>
                                            @endforeach
                        ...
                        <div class="modal-footer bg-light d-flex gap-2">
                            <button type="button" class="btn btn-secondary btn-sm flex-grow-1 fw-semibold"
                                data-bs-dismiss="modal">Tutup</button>
                            @if ($s->details->count() > 0 && \Carbon\Carbon::parse($s->created_at)->diffInDays(now()) <= 2)
                                <button type="submit" class="btn btn-primary btn-sm flex-grow-1 fw-bold shadow-sm"><i
                                        class="fas fa-calculator me-1"></i> Simpan & Hitung Ulang</button>
                            @endif
                        </div>
                    </form>
                </div>
        ...
        @if ($s->status == 'pending')
            <div class="modal fade" id="modalSetujuTrx-{{ $s->id }}" tabindex="-1"
                aria-labelledby="modalSetujuLabel-{{ $s->id }}" aria-hidden="true" data-bs-backdrop="static">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content shadow-lg border-0">
                        <div class="modal-header bg-success text-white">
                            <h5 class="modal-title fw-bold" id="modalSetujuLabel-{{ $s->id }}"><i
                                    class="fas fa-check-double me-2"></i> Konfirmasi Validasi Fisik</h5>
                            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"
                                aria-label="Close"></button>
                        </div>
                        ...
                        <div class="modal-footer bg-light d-flex gap-2">
                            ...
                            <form action="{{ route('transaksi.update-status', $s->id) }}" method="POST"
                                class="flex-grow-1 p-0 m-0">
                                @csrf @method('PATCH')
                                <input type="hidden" name="status" value="success">
                                <button type="submit"
                                    class="btn btn-success btn-sm w-100 fw-bold text-white shadow-sm"><i
                                        class="fas fa-check me-1"></i> Ya, Sudah Sesuai</button>
                            </form>
                        </div>
                    </div>
                </div>
